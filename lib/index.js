...
Mongueue.prototype.enqueue = function(item, schedule, callback) {
    ...
    if (schedule && schedule.insertat) {
        ...
        var now = new Date();
        // clean the reoccurrence part on the item, to assist is reoccurence item query
        this.clearDailyIfEmpty(schedule);
    ...
    var entry = {
        ...
        attempts : 0,
        maxAttempts: (item.maxAttempts !== undefined ? item.maxAttempts : this.MaxAttempts),
        disabled: false
    };
...
// clear the daily scheduling if empty or corrupted
Mongueue.prototype.clearDailyIfEmpty = function(schedule) {
    var i = 0, toDelete = true;
    if (!schedule || !schedule.daily) return;

    if (!(schedule.daily instanceof Array) || schedule.daily.length !== 7) return delete schedule['daily'];
    for (i = 0; i < 7; i++) {
        // if item is not a list of ints clear
        if (!(schedule.daily[i] instanceof Array)) return delete schedule['daily'];

        // have scheduling on the ith day, don't clean
        if (schedule.daily[i].length > 0) toDelete = false;
    }

    // got here - daily is empty or corrupted delete the daily section
    if (toDelete) {
...
Mongueue.prototype.dequeue = function(ttl, callback) {
  ...
  this.collection.findAndModify(
    { $and :[{$or: [ { hiddenUntil: null }, { hiddenUntil: { $lt: now } } ]}, { disabled: false }] },  // query
      [[ 'createdAt', 'asc']],                                          // sort by old to new
      { $set: { hiddenUntil: hiddenUntil }, $inc : {attempts : 1 } },   // update
   ...
   , function(err, item) {
    ...
    function releaseFn(err, cb) {
      ...
      if (err && err.errcode !== self.AttemptsExceeded) {
        if (cb) cb();
        return;
      } 
    ...
    function disableFn(err, cb) {
        // disable a message
        self.disable(item._id, cb || function() {});
    }
    ...
    function requeueFn(err, cb) {
      ...
      coll.findAndModify(
        ...
        { $set: { hiddenUntil: revival, attempts : 0 } }, // set to revival
    ...
    if (item.attempts > item.maxAttempts) {
        // item passed the attempts limit, remove from the queue
        console.warn(item._id, " attempted too many times, disable it");
        return callback({errcode: self.AttemptsExceeded, errmsg: 'max attempts exceeded'}, item, disableFn, item._id);
    }
    ...
    if (!item.reoccurrence || !item.reoccurrence.length == 0 || isSingle) {
...
// Disable an item
//
Mongueue.prototype.disable = function(id, cb) {
    cb = cb || function() {};
    id = (typeof id == 'string' ? new bson.ObjectID(itemId) : id);
    this.collection.findAndModify({ _id: id },[], { $set: { disabled: true }}, { "new"  : false }, cb);
}

//
// Enable an item
//
Mongueue.prototype.enable = function(id, cb) {
    cb = cb || function() {};
    id = (typeof id == 'string' ? new bson.ObjectID(itemId) : id);
    this.collection.findAndModify({ _id: id },[], { $set: {disabled: false, attempts: 0}}, {"new"  : false}, cb);
}
...
// Peeks the top messages on the queue
// top: number on items to peek
// callback(err, items)
Mongueue.prototype.top = function (top, callback) {
    if (!callback) return;
    var i,
        cursor = this.collection.find({ })       // query (empty - return all)
        .sort({'hiddenUntil': 1})   // sort by old to new
        .limit(top)
        .toArray(callback);
};

// Peeks the top messages on the queue that are repeating
// top: number on items to peek
// callback(err, items)
Mongueue.prototype.topRepeat = function (top, callback) {
    if (!callback) return;
    var i,
        cursor = this.collection
            .find({ $where: "this.reoccurrence != undefined && this.reoccurrence.daily != undefined && this.reoccurrence.single != true"})
            .sort({'hiddenUntil': 1})   // sort by old to new
            .limit(top)
            .toArray(callback);

    // TODO: convert to $and when v2.0 is intalled:
    //({ $and: [{reoccurrence : { $ne : undefined }}, {'reoccurrence.daily' : { $ne : undefined }}, {'reoccurrence.single' : { $ne : true}}]})
    // OR
    // ({ $and: [{reoccurrence : { $exists : true }}, {"reoccurrence.daily" : { $exists : true }}, {$or: [{"reoccurrence.single": { $exists : false }}, {"reoccurrence.single": { $ne : false }}]})]})       // query (empty - return all)
};

// Peeks the top messages on the queue that are disabled
// top: number on items to peek
// callback(err, items)
Mongueue.prototype.topDisabled = function (top, callback) {
    if (!callback) return;
    var i,
        cursor = this.collection
            .find({ disabled: true })
